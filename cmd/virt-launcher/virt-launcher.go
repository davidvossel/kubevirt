...
func waitForDomainUUID(timeout time.Duration, events chan watch.Event, stop chan struct{}, domainManager virtwrap.DomainManager, virtShareDir string) *api.Domain {

	ticker := time.NewTicker(timeout).C
	checkEarlyExit := time.NewTicker(time.Second * 2).C

	annotationsFile := filepath.Join(virtShareDir, "podinfo", "annotations")
	...
	for {
		...
		case <-checkEarlyExit:
			if util.HasEarlyExitAnnotation(annotationsFile) {
				panic(fmt.Errorf("received early exit signal via annotation at path %s before domain was detected", annotationsFile))
			}
		case <-stop:
			return nil
		}
...
func main() {
	...
	domain := waitForDomainUUID(*qemuTimeout, events, signalStopChan, domainManager, *virtShareDir)
	if domain != nil {
