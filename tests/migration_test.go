			...
			It("Target pod should exit after failed migration", func() {
				...
				Eventually(func() error {
					...
					if pod.Status.Phase != k8sv1.PodFailed {
						return fmt.Errorf("still waiting on target pod to fail, current phase is %s", pod.Status.Phase)
					}

					return nil
				}, 10*time.Second, time.Second).Should(Succeed(), "Target pod should fail quickly after migration fails.")
