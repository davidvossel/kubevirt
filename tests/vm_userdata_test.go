...
var _ = Describe("CloudInit", func() {
		...
		for _, pod := range pods.Items {
			...
			for _, containerStatus := range pod.Status.ContainerStatuses {
				if strings.Contains(containerStatus.Name, "config-disk") == false {
					// only check readiness of config-disk containers
					continue
				}
	...
	Context("CloudInit ConfigDisk UserData", func() {
		It("should launch multiple VMs with cloud-init configDisks", func(done Done) {
			num := 2
			vms := make([]*v1.VM, 0, num)
			objs := make([]runtime.Object, 0, num)
			for i := 0; i < num; i++ {
				vm := tests.NewRandomVMWithUserData("kubevirt/cirros-registry-disk-demo:devel")
				obj := LaunchVM(vm)
				vms = append(vms, vm)
			...
			close(done)
		}, 45) // Timeout is long because this test involves multiple parallel VM launches.
	})
})
