...
var _ = Describe("[rfe_id:1177][crit:medium][vendor:cnv-qe@redhat.com][level:component]VirtualMachine", func() {
	...
	Context("[Serial]A mutated VirtualMachine given", func() {
		...
		It("Hotplug a volume on an offline VM", func() {
			By("Creating VirtualMachine")
			template, _ := newVirtualMachineInstanceWithContainerDisk()
			vm := createVirtualMachine(false, template)
			...
			err = virtClient.VirtualMachine(createdVM.Namespace).AddVolume(createdVM.Name, &v1.AddVolumeOptions{
				Name: "some-new-volume1",
				Disk: &v1.Disk{
					DiskDevice: v1.DiskDevice{
						Disk: &v1.DiskTarget{
							Bus: "scsi",
			...
			Eventually(func() error {
				...
				if len(updatedVM.Status.VolumeRequests) > 0 {
					return fmt.Errorf("waiting on all VolumeRequets to be processed")
				}
				...
				for _, volume := range updatedVM.Spec.Template.Spec.Volumes {
					if strings.Contains(volume.Name, "some-new-volume") {
						foundVolume++
					}
				}
				for _, disk := range updatedVM.Spec.Template.Spec.Domain.Devices.Disks {
					if strings.Contains(disk.Name, "some-new-volume") {
						foundDisk++
					}
				}
			...
			err = virtClient.VirtualMachine(createdVM.Namespace).RemoveVolume(createdVM.Name, &v1.RemoveVolumeOptions{
				Name: "some-new-volume1",
			})
			Expect(err).ToNot(HaveOccurred())
			err = virtClient.VirtualMachine(createdVM.Namespace).RemoveVolume(createdVM.Name, &v1.RemoveVolumeOptions{
