...
func callback2(d cli.VirDomain, event *libvirt.DomainEventLifecycle, client *notifierserver.DomainNotifierClient) {

	domain, err := NewDomain(d)
	if err != nil {
		log.Log.Reason(err).Error("Could not create the Domain.")
		client.DomainEvent(newWatchEventError(err))
		return
	}
	...
	case api.ReasonNonExistent:
		client.DomainEvent(watch.Event{Type: watch.Deleted, Object: domain})
	default:
		if event.Event == libvirt.DOMAIN_EVENT_DEFINED && libvirt.DomainEventDefinedDetailType(event.Detail) == libvirt.DOMAIN_EVENT_DEFINED_ADDED {
			client.DomainEvent(watch.Event{Type: watch.Added, Object: domain})
		} else {
			client.DomainEvent(watch.Event{Type: watch.Modified, Object: domain})
		}
	}
}

func StartNotifier(domainConn cli.Connection) error {
	...
	entrypointCallback := func(c *libvirt.Connect, d *libvirt.Domain, event *libvirt.DomainEventLifecycle) {
		...
		client, err := notifierserver.NewDomainNotifierClient("/var/run/kubevirt/domain-notify.sock")
		if err != nil {
		...
		if event == nil {
			client.DomainEvent(newWatchEventError(fmt.Errorf("Libvirt reconnect")))
			return
		}
		callback2(d, event, client)
	}
...
func newNotifierWatcher() (watch.Interface, error) {
	watcher := &DomainWatcher{C: make(chan watch.Event)}

	// TODO follow pattern in inotify to start/stop background
	go func() {
		notifierserver.Run("/var/run/kubevirt/domain-notify.sock", watcher.C)
	}()
	return watcher, nil
}

// TODO move this function into virt-launcher/cache/cache
// get rid of cycle
func newListWatchFromNotifier() *cache.ListWatch {
	listFunc := func(options metav1.ListOptions) (runtime.Object, error) {
		log.Log.V(3).Info("Synchronizing domains")

		// TODO actually list all known domains some how
		list := api.DomainList{
			Items: []api.Domain{},
		}
