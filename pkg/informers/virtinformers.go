...
type KubeInformerFactory interface {
	...
	// Watches for pods created as a result of VM objects.
	VMPods() cache.SharedIndexInformer
	// Watches for pods created as a vm target during migration
	MigrationTargetPods() cache.SharedIndexInformer
	// Watches for migration job pods
	MigrationJobs() cache.SharedIndexInformer
}
...
func (f *kubeInformerFactory) MigrationTargetPods() cache.SharedIndexInformer {
	return f.getInformer("migrationTargetPods", func() cache.SharedIndexInformer {
		...
		lw := kubecli.NewListWatchFromClient(f.clientSet.CoreV1().RESTClient(), "pods", kubeapi.NamespaceDefault, fieldSelector, labelSelector)
		return cache.NewSharedIndexInformer(lw, &k8sv1.Pod{}, f.defaultResync, cache.Indexers{})
	})

}

func (f *kubeInformerFactory) VMPods() cache.SharedIndexInformer {
	return f.getInformer("vmPodInformer", func() cache.SharedIndexInformer {
		fieldSelectionQuery := fmt.Sprintf("status.phase=%s", string(kubeapi.PodRunning))
		fieldSelector := fields.ParseSelectorOrDie(fieldSelectionQuery)
		labelSelectorQuery := fmt.Sprintf("!%s, %s in (virt-launcher)", string(kubev1.MigrationLabel), kubev1.AppLabel)
		labelSelector, err := labels.Parse(labelSelectorQuery)
...
func (f *kubeInformerFactory) MigrationJobs() cache.SharedIndexInformer {
	return f.getInformer("migrationJobInformer", func() cache.SharedIndexInformer {
