...
const (
	...
	ConditionReasonDeploymentSuccessful     = "DeploymentSuccessful"
	ConditionReasonDeletionFailedError      = "DeletionFailed"
	ConditionReasonDeletionSuccessful       = "DeletionSuccessful"
)
...
func (c *KubeVirtController) execute(key string) error {
	...
	if kv.DeletionTimestamp != nil {
		...
		if err != nil {
			// deletion failed
			err := util.UpdateCondition(kv, v1.KubeVirtConditionDeletionSuccessful, k8sv1.ConditionFalse, ConditionReasonDeletionFailedError, fmt.Sprintf("An error occurred during deletion: %v", err), c.clientset)
			if err != nil {
		...
		err = util.UpdateCondition(kv, v1.KubeVirtConditionDeletionSuccessful, k8sv1.ConditionTrue, ConditionReasonDeletionSuccessful, "Deletion successful", c.clientset)
		if err != nil {
	...
	for _, obj := range kvs {
		if fromStore, ok := obj.(v1.KubeVirt); ok {
			...
			if isKubeVirtActive(&fromStore) {
				...
				err := util.UpdateCondition(kv, v1.KubeVirtConditionDeploymentSuccessful, k8sv1.ConditionFalse, ConditionReasonDeploymentFailedExisting, "There is an active KubeVirt deployment", c.clientset)
				if err != nil {
					log.Log.Errorf("Failed to set condition: %v", err)
				}
				return nil
	...
	if err != nil {
		...
		util.UpdateCondition(kv, v1.KubeVirtConditionDeploymentSuccessful, k8sv1.ConditionFalse, ConditionReasonDeploymentFailedError, fmt.Sprintf("Failed to add finalizer: %s", err), c.clientset)
		return err
	}
	...
	if err != nil {
		// deployment failed
		err := util.UpdateCondition(kv, v1.KubeVirtConditionDeploymentSuccessful, k8sv1.ConditionFalse, ConditionReasonDeploymentFailedError, fmt.Sprintf("An error occurred during deployment: %v", err), c.clientset)
		if err != nil {
			log.Log.Errorf("Failed to set condition: %v", err)
		}
		return err
	...
	err = util.UpdateCondition(kv, v1.KubeVirtConditionDeploymentSuccessful, k8sv1.ConditionTrue, ConditionReasonDeploymentSuccessful, "Deployment successful", c.clientset)
	if err != nil {
		log.Log.Errorf("Failed to update condition: %v", err)
	}
	return nil
