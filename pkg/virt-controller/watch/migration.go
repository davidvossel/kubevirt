...
func (c *MigrationController) updateStatus(migration *virtv1.VirtualMachineInstanceMigration, vmi *virtv1.VirtualMachineInstance, pods []*k8sv1.Pod) error {
	...
	if migration.IsFinal() {
		...
		case virtv1.MigrationRunning:
			_, exists := pod.Annotations[virtv1.MigrationTargetStartTimestamp]
			if !exists && vmi.Status.MigrationState.TargetNodeDomainStartTimestamp != nil {
				...
				patchOps := fmt.Sprintf(`[{ "op": "add", "path": "/metadata/annotations/%s", "value": "%s" }]`,
					key,
					vmi.Status.MigrationState.TargetNodeDomainStartTimestamp.String())
