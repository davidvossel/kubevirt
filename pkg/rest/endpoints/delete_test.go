...
func newValidDeleteRequest() *http.Request {
	request, _ := http.NewRequest("DELETE", "/apis/kubevirt.io/v1/namespaces/default/virtualmachineinstances/test", nil)
	return request
}
...
var _ = Describe("Delete", func() {
	...
	BeforeEach(func() {
		...
		ws.Route(ws.DELETE("/apis/kubevirt.io/v1/namespaces/{namespace}/virtualmachineinstances/{name}").To(target))
	...
	Describe("REST call", func() {
		...
		Context("with valid request", func() {
			...
			It("should detect labelSelector", func() {
				request, _ := http.NewRequest("DELETE", "/apis/kubevirt.io/v1/namespaces/default/virtualmachineinstances/test?labelSelector=app%3Dmyapp", nil)
				handler.ServeHTTP(recorder, request)
				responseBody := payload{}
				json.NewDecoder(recorder.Body).Decode(&responseBody)
