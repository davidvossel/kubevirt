...
func newValidPutRequest() *http.Request {
	request, _ := http.NewRequest("PUT", "/apis/kubevirt.io/v1/namespaces/default/virtualmachineinstances/test", nil)
...
var _ = Describe("Put", func() {
	...
	BeforeEach(func() {
		...
		ws.Route(ws.PUT("/apis/kubevirt.io/v1/namespaces/{namespace}/virtualmachineinstances/{name}").To(target))
