...
func newValidGetRequest() *http.Request {
	request, _ := http.NewRequest("GET", "/apis/kubevirt.io/v1/namespaces/default/virtualmachineinstances/test", nil)
	return request
}
...
var _ = Describe("Get", func() {
	...
	BeforeEach(func() {
		...
		ws.Route(ws.GET("/apis/kubevirt.io/v1/namespaces/{namespace}/virtualmachineinstances/{name}").Produces(rest.MIME_JSON, rest.MIME_TEXT, rest.MIME_YAML).To(target))
	...
	Describe("REST call", func() {
		...
		Context("with valid request", func() {
			...
			It("should detect labelSelector", func() {
				request, _ := http.NewRequest("GET", "/apis/kubevirt.io/v1/namespaces/default/virtualmachineinstances/test?labelSelector=app%3Dmyapp", nil)
				handler.ServeHTTP(recorder, request)
				responseBody := payload{}
				json.NewDecoder(recorder.Body).Decode(&responseBody)
