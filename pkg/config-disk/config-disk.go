...
func (c *configDiskClient) Define(vm *v1.VM) error {
	...
	if vm.Spec.CloudInit != nil {
		...
		response, err = c.unixClient.Post("http:///create/cloudinit/"+namespace+"/"+domain, "application/json", strings.NewReader(string(body)))
...
func (c *configDiskClient) Undefine(vm *v1.VM) error {
	...
	var response *http.Response
	var err error
	response, err = c.unixClient.Post("http:///delete/all/"+namespace+"/"+domain, "", nil)
